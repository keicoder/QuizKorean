//
//  CustomDraggableModalTransitionAnimator.m
//


#import "CustomDraggableModalTransitionAnimator.h"

@interface CustomDraggableModalTransitionAnimator ()

@property (nonatomic, strong) UIViewController *modalController;
@property (nonatomic, strong) DetectScrollViewEndGestureRecognizer *gesture;
...
@implementation CustomDraggableModalTransitionAnimator
...
- (instancetype)initWithModalViewController:(UIViewController *)modalViewController
{
    ...
    if (self) {
        ...
        _direction = ModalTransitonDirectionBottom;
        _dragable = NO;
        _bounces = YES;
        _behindViewScale = 0.9f;
        _behindViewAlpha = 0.7f;
...
- (void)setDragable:(BOOL)dragable
{
    ...
    if (self.isDragable) {
        self.gesture = [[DetectScrollViewEndGestureRecognizer alloc] initWithTarget:self action:@selector(handlePan:)];
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    if (!self.isDismiss) {
        ...
        if (self.direction == ModalTransitonDirectionBottom) {
        ...
        } else if (self.direction == ModalTransitonDirectionLeft) {
            startRect = CGRectMake(-CGRectGetWidth(containerView.frame),
                                   0,
                                   CGRectGetWidth(containerView.bounds),
                                   CGRectGetHeight(containerView.bounds));
        } else if (self.direction == ModalTransitonDirectionRight) {
            startRect = CGRectMake(CGRectGetWidth(containerView.frame),
                                   0,
                                   CGRectGetWidth(containerView.bounds),
                                   CGRectGetHeight(containerView.bounds));
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (velocityForSelectedDirection > 100
            && (self.direction == ModalTransitonDirectionRight
                || self.direction == ModalTransitonDirectionBottom)) {
                [self finishInteractiveTransition];
            } else if (velocityForSelectedDirection < -100 && self.direction == ModalTransitonDirectionLeft) {
                [self finishInteractiveTransition];
            } else {
...
#pragma mark - DetectScrollViewEndGestureRecognizer
